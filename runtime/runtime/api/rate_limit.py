...
class RateLimiterPort(Protocol):
    async def allow(self, api_key: str) -> bool: ...


class MemoryRateLimiter:
    """Fixed per-minute window (tests and Redis-down fallback)."""
...
async def check_rate_limit(api_key: str) -> None:
    settings = Settings()
    if settings.rate_limit_at_gateway_only or not settings.rate_limit_enabled:
        return
    allowed = await get_rate_limiter().allow(api_key)
    if not allowed:
        raise RateLimitedError(limit=settings.rate_limit_per_minute)
